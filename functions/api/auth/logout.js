...
import { revokeSession, clearSessionCookie, isSecureRequest } from '../_session.js'

export async function onRequestPost({ request, env }) {
  if (!originAllowed(request)) return json({ success: false, error: 'Bad origin.' }, 403)
  await revokeSession(env, request)
  return json({ success: true }, 200, { 'Set-Cookie': clearSessionCookie({ secure: isSecureRequest(request) }) })
}
