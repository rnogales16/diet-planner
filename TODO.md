...
Paso (b) hecho (backend): tablas `email_verification_tokens`/`password_reset_tokens` (hash,
un solo uso, TTL, CASCADE); `_email.js` (Resend + fallback dev que loguea el enlace);
`_token.js`; `resolveUser` devuelve `emailVerified` (Access⇒true); endpoints
`/api/auth/{verify-email,resend-verification,forgot-password,reset-password}`; `register.js`
envía verificación best-effort; `generate-meal-plan.js` migrado a `resolveUser` + **capado 403
hasta email verificado**. Rate-limit por email (primario) + IP (secundario) en resend/forgot.

- [ ] ⚠️ **Prod: configurar Resend** — `RESEND_API_KEY` (secret) y `EMAIL_FROM` (remitente de
  dominio verificado en Resend) en Pages → Environment variables (Production/Preview). En
